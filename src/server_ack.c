...
int main(int argc, char *argv[])
{
	...
	*******************************************************************************************************/
	while (1) 
	{
        ...
        // search in larger ack array!
		else if(type == 1)
		{
			struct packet* data_packet = (struct packet*)buf;
			// process only if we haven't received the packet beforehand
			if(fullAck.full_packet_tracker[data_packet->sequence_number] <= 0) 
			{
				...
				fullAck.full_packet_tracker[data_packet->sequence_number] = 1; // update ack sequence
				packet_counter++;
			}
		}
		...
		else if(type == 2)
		{
			...
			else
			{
                ...
                {
                    ack_packet1.sequenceNumber = i;
                    // grab data by offset of data_size * i
                    memcpy(ack_packet1.packet_tracker, (uint8_t*)&fullAck.full_packet_tracker + data_size * i, sizeof(ack_packet1.packet_tracker));
                    memcpy(send_buffer,(unsigned char*)&ack_packet1,sizeof(ack_packet1));
                    n = sendto(sock,send_buffer,sizeof(ack_packet1),0,(struct sockaddr *)&from,fromlen);
                    if (n  < 0) error("sendto");
                }
